            ...
            <div :class="{ 'hidden': page !== 2 }">
                ...
                <div id="question-container" class="overflow-x-auto p-4">
                    @foreach ($survey->questions->slice(0, 6) as $question)
                        <div class="mb-6 p-6 bg-white shadow-lg rounded-lg border border-gray-300">
                            <h4 class="text-2xl font-semibold text-green-700 mb-4">{{ $question->question }}</h4>

                            <table class="min-w-full table-auto border-collapse border border-gray-300">
                                <thead>
                                    <tr class="bg-green-50">
                                        <th class="px-4 py-2 text-left font-medium text-green-700">Instructor</th>

                                        @foreach ($question->options as $option)
                                            <th class="px-2 py-2 text-center text-sm text-green-700">{{ $option }}</th>
                                        @endforeach
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($instructors as $instructor)
                                        <tr class="border-b hover:bg-green-50">
                                            <td class="px-4 py-3 text-sm text-gray-800 w-1/3">
             ...
             <div :class="{ 'hidden': page !== 6 }">
                ...
                <div id="question-container">
                    @foreach ($survey->questions->slice(20, 2) as $question)
                        <div class="mb-4">
                            <h4 class="text-lg font-semibold text-gray-700 mb-2">{{ $question->question }}</h4>
                            @foreach ($instructors as $instructor)
                                <div class="mb-6">
                                    <p class="text-sm font-semibold text-gray-800"><strong>Instructor: {{ $instructor->first_name }} {{ $instructor->last_name }}</strong></p>
                                    <div class="w-full text-sm text-left border border-gray-300 rounded-lg p-4">
                                        ...
                                        @elseif($question->type == 'text')
                                            <textarea name="answers[{{ $instructor->id }}][{{ $question->id }}]"
                                                      ...
                                                      class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:outline-none resize-none text-gray-700 placeholder-gray-400">
                                            </textarea>
                                        @endif
                                    </div>
                                </div>
